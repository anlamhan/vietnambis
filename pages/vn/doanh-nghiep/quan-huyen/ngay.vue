<template>
  <div id="app" class="container">
    ...
    <div class="title">
        <h1>Danh sách công ty có người đại diện tên là {{this.date}}</h1>
    </div>
  ...
  <vue-page :total="count" :page="page" :path="`/vn/doanh-nghiep/quan-huyen/${this.slug}/ngay${this.date}/`" model="link"></vue-page>
  </div>
</template>
<script>
...
export default {  
 ...
 async asyncData({ params, error }) {   
     ...
     console.log('ngay:'+params.date)      
    let page = parseInt(params.page) || 0
    var host = process.env.baseUrl;    
    var slug = params.id+'-'+params.slug;
    let [businessRes, provinceRes, countRes] = await Promise.all([
      axios.get(`${host}/api/business/getdaybusinessbydistrict/${params.id}/${params.date}/${page}`),
      axios.get(`${host}/api/province/list`),
      axios.get(`${host}/api/business/countdaybusinessbydistrict/${params.id}/${params.date}`)
    ]).catch(err => {
