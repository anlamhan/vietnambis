    ...
    <div>
        <template v-if="pageAll > 1">
            <template v-if="model == 'link'">
                <nuxt-link :class="{disable:pageNow == 1}" :to="prevPage">&larr;Trang trước</nuxt-link>
                <nuxt-link :class="{disable:pageNow >= pageAll-1}" :to="nextPage">Trang sau&rarr;</nuxt-link>
            </template>
            <template v-else>
                <button :class="{disable:pageNow == 0}" @click="prev">&larr;Trang trước</button>
                <button :class="{disable:pageNow >= pageAll-1}" @click="next">Trang sau&rarr;</button>
            </template>
        </template>
    </div>
</template>
