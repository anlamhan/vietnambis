...
export function businessToSQLUpdate(object) {
  let updated = [], params = [];
  for (var key in object) {
    if (object.hasOwnProperty(key)) {
      if (key === 'id') {
...
export function timeAgo(time) {
  ...
  if (between < 3600) {
    return pluralize(~~(between / 60), ' Vài phút trước')
  } else if (between < 86400) {
    return pluralize(~~(between / 3600), ' Vài giờ trước')
  } else {
    return pluralize(~~(between / 86400), ' Ngày trước')
  }
}
